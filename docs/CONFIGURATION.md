...
Wireseer loads TOML from the operating system's application configuration directory. Use
`wireseer config path` for the exact location and `wireseer config init` to write current defaults.
...
- `subnet`: optional IPv4 CIDR. Without it, Wireseer uses the selected interface's network.
...
- `theme`: `wireseer_dark`, `catppuccin_mocha`, `catppuccin_latte`, `dracula`, `nord`,
...
- `icons`: `unicode` is the safe default, `ascii` maximizes compatibility, and `nerd` uses
  ...
  reliably. `--icons MODE` or `WIRESEER_ICONS=MODE` overrides this value for one run.
...
- `vendor_database`: optional local IEEE CSV or OUI text file. `wireseer vendor update` installs the
  official IEEE public registries; `wireseer vendor import` accepts another source. Both validate a
